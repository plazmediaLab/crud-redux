...
const Products = props => {
  ...
  return (
    <Fragment>
      {error ? (
        <p className="msn msn-l-cancel txt-a-c mb-5">
          <i className="a-refresh"></i> There was an error to load the products,
          try refresh site
        </p>
      ) : null}
      {loading ? (
        <p className="msn msn-l-blue txt-a-c mb-5">
          <i className=" a-hourglass_empty i-spin-l"></i>&nbsp; Loading...
        </p>
      ) : null}
        ...
        <table className="table ab-even t-hover">
          <TheadContain className="">
            <tr>
              ...
              <th>Price</th>
              <th>
                <div className="jc-end">Actions</div>
              </th>
            </tr>
          </TheadContain>
          <tbody>
            {products.length === 0 ? (
              ...
              products.map(item => {
                ...
                return <Product key={item.id} item={item} count={count} />;
              })
            )}
          </tbody>
        </table>
